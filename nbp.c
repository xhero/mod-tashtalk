...
int netddp_open(struct sockaddr_at *addr, struct sockaddr_at *bridge) {
    ...
    if ((s = socket(AF_APPLETALK, SOCK_DGRAM, 0)) < 0) return -1;

	int opt = 1;
	setsockopt(s, SOL_SOCKET, SO_DEBUG, (char *)&opt, sizeof(opt));
...
static void done(int sig) {
    if (nsent) {
        printf("\n----%d.%d AEP Statistics----\n", ntohs(target.sat_addr.s_net),
               target.sat_addr.s_node);
        ...
        if (nrecv) {
            printf("round-trip (ms)  min/avg/max = %ld/%ld/%ld\n",
                   (unsigned long)minms, (unsigned long)totalms / nrecv,
                   (unsigned long)maxms);
        }
    }
    exit(0);
...
int main(int ac, char **av) {
    ...
    saddr.sat_addr.s_node = 2;
    saddr.sat_addr.s_net = 0;
    saddr.sat_port = 128;
	...
	target.sat_addr.s_node = 56;
    target.sat_addr.s_net = 0;

    if ((sock = netddp_open(
             saddr.sat_addr.s_net || saddr.sat_addr.s_node ? &saddr : NULL,
             NULL)) < 0) {
        perror("ddp_open");
        exit(1);
    }
    ...
    for (;;) {
        ...
        nrecv++;
        ms =
            (tv.tv_sec - atv.tv_sec) * 1000 + (tv.tv_usec - atv.tv_usec) / 1000;
        totalms += ms;
        if (ms > maxms) {
            maxms = ms;
        }
        ...
        printf("%d bytes from %u.%u: aep_seq=%d. time=%ld. ms\n", cc,
               ntohs(sat.sat_addr.s_net), sat.sat_addr.s_node, seq,
               (unsigned long)ms);
